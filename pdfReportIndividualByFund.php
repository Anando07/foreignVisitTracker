...
$fundReqEncoded = $_GET['fundReq'];   
$fundReq = urldecode($fundReqEncoded);
...
//$sql = "SELECT * from ForeignVisit WHERE FundingSource = '" . $fundReq . "' ORDER BY StartDate desc";
$sql = "SELECT * from ForeignVisit WHERE FundingSource = '" . $fundReq . "' AND Office = '" . $officeReq . "' ORDER BY StartDate desc";
...
$pdf->Ln();	
//$pdf->Cell(0,10,'Foreign Visit Record by Name',0.0,0.0,'C');
$pdf->Cell(0,10, $officeReq . " Foreign Visits funded by ".$fundReq." from ".$startDate." to ".$endDate,1.0,0.0,'C');
$pdf->SetFont('Arial','',8);
$pdf->Ln();	
...
if($query->num_rows > 0)
{
	...
	while($row = $query->fetch_assoc()) { 
		...
		//$pdf->Cell(50,7,$row["Workplace"] . ", " . $row["Office"],1,0,'C');
