...
$fundReq = $_POST['fundReq'];
$officeReq = $_POST['officeReq']; 
$startDate = $_POST['FromDate'];
$endDate = $_POST['ToDate'];

if($fundReq == ""){
	die("<br><br><center> Sorry. Fund field cannot be left empty. <center><br><br>");
}
...
$fundReqEncoded = urlencode($fundReq);
$fundReqDecoded = urldecode($fundReq);
//echo '<a href=pdfReportIndividualByFund.php?fundReq='.$fundReqEncoded.'&FromDate='.$startDate.'&ToDate='.$endDate.' target="_blank"> <p style="text-align:center"> Download Report </p></a>'; 
echo '<a href=pdfReportIndividualByFund.php?fundReq='.$fundReqEncoded.'&officeReq='.$officeReq.'&FromDate='.$startDate.'&ToDate='.$endDate.' target="_blank"> <p style="text-align:center"> Download Report </p></a>'; 
//echo '<a href = uploads.html?id='.$nameReqEncoded.'> Link </a>';

//$sql1 = "SELECT * from ForeignVisit WHERE FundingSource = '" . $fundReq . "' ORDER BY StartDate desc"; 
$sql1 = "SELECT * from ForeignVisit WHERE FundingSource = '" . $fundReq . "' AND Office = '" . $officeReq . "' ORDER BY StartDate desc";
//$sql1 = "SELECT * from ForeignVisit WHERE FundingSource = '" . $fundReq . "' ORDER BY StartDate desc LIMIT $results_per_page OFFSET $start_from"; 
//$sql2 = "SELECT * FROM Census WHERE UserID = '$word' ORDER BY ID DESC LIMIT $results_per_page OFFSET $start_from";

$result = mysqli_query($db,$sql1);
if ($result->num_rows > 0) {
    // output data of each row  
    //echo "<br>";
    echo "<center> <b> <u> Fund Search Result (" . $fundReq . ")" . "</u> </b> </center> <br>";
