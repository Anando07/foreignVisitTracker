...
if($idReq == ""){
	die("<br><br><center> Sorry. ID field cannot be left empty. <center><br><br>");
}
...
echo '<a href=pdfReportIndividualByID.php?idReq='.$idReqEncoded.'&FromDate='.$startDate.'&ToDate='.$endDate.' target="_blank"> <p style="text-align:center"> Download Report </p></a>'; 
//echo '<a href = uploads.html?id='.$nameReqEncoded.'> Link </a>';

//$sql1 = "SELECT * from ForeignVisit WHERE Name LIKE '" ."%" . $nameReq . "%" . "'"; 
$sql1 = "SELECT * from ForeignVisit WHERE ServiceID = " . $idReq;
...
if ($result->num_rows > 0) { 
    ...
    //echo "<br>";
    echo "<center> <b> <u> ID Search Result (" . $idReq .")</u> </b> </center> <br>";
