...
$officeReqEncoded = $_GET['officeReq'];   
$officeReq = urldecode($officeReqEncoded);
...
$sql = "SELECT * from ForeignVisit WHERE Office = '" . $officeReq . "' ORDER BY StartDate desc";
...
$pdf->SetTextColor(0,0,0); 
//$pdf->Cell(0,10,'Foreign Visit Record by Name',0.0,0.0,'C');
$pdf->Cell(0,6,'Foreign Visit Record of '.$officeReq. " from ".$startDate ." to ".$endDate,1.0,0.0,'C');
$pdf->Ln();	
//$pdf->Cell(0,10," from ".$startDate ." to ".$endDate,0.0,0.0,'C');
$pdf->SetFont('Arial','',8);
$pdf->Ln();	
$pdf->Cell(0,4,'(IO = International Organisation, FS = Foreign Scholarship, BS = Bangladeshi Scholarship, EBL = Ex-Bangladesh Leave, EL = Extraordinary Leave)',0,1,'C');
...
if($query->num_rows > 0)
{
	...
	while($row = $query->fetch_assoc()) { 
		...
		//$pdf->Cell(46,7,$row["Workplace"] . ", " . $row["Office"],1,0,'C');
...
//$filename="/Applications/MAMP/htdocs/testreport.pdf";
