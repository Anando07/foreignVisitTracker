...
//$sql = "SELECT * from ForeignVisit WHERE ActualArrival = 0000-00-00 OR ActualDeparture = 0000-00-00 ORDER BY StartDate desc";
//$sql = "SELECT * from ForeignVisit WHERE ActualArrival = 0000-00-00 OR ActualDeparture = 0000-00-00 ORDER BY EndDate ASC"; 
$sql = "SELECT * from ForeignVisit WHERE ActualArrival = 0000-00-00 AND EndDate < CURDATE() ORDER BY EndDate ASC"; 
...
$pdf->Ln();	
$pdf->SetTextColor(255,0,0);
$pdf->Cell(0,10,'Unreported Cases',1.0,0.0,'C');
$pdf->SetTextColor(0,0,0);
$pdf->SetFont('Arial','',8);
$pdf->Ln();	
...
//$pdf->Cell(16,7,"From",1,0,'C');
$pdf->Cell(16,7,"End Date",1,0,'C');
...
if($query->num_rows > 0)
{
	...
	while($row = $query->fetch_assoc()) { 
		$pdf->SetFont('Arial','',7);	
		$pdf->Ln();
		$pdf->Cell(10,7,$row["ServiceID"],1,0,'C');
		$pdf->Cell(48,7,$row["Name"],1,0,'C');
		//$pdf->Cell(57,7,$row["Designation"]. " (Grade-" . $row["Grade"] . "), " . $row["Office"],1,0,'C');
		...
		$pdf->Cell(16,7,$row["StartDate"],1,0,'C');
		//$pdf->Cell(16,7,$row["EndDate"],1,0,'C');
