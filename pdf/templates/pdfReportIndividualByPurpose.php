...
if (!empty($startDate) && !empty($endDate)) {
    ...
    $dateE = date('Y-m-d', strtotime($endDate));

    if ($dateE < $dateS) {
        die("<br><br><center>Sorry. To Date must be later than From Date.</center><br><br>");
    }

    $dateCondition = " AND StartDate BETWEEN '$dateS' AND '$dateE'";
} elseif (!empty($startDate)) {
    $dateS = date('Y-m-d', strtotime($startDate));
    $dateCondition = " AND StartDate >= '$dateS'";
} elseif (!empty($endDate)) {
    $dateE = date('Y-m-d', strtotime($endDate));
    $dateCondition = " AND StartDate <= '$dateE'";
}
...
$sql = "SELECT * FROM ForeignVisit
        WHERE Purpose = '$purposeReq' $dateCondition
...
$pdf->Cell(0,10,"Foreign Visit Record of $purposeReq ($titleDate)",1,1,'C');
