    ...
    while($row = $result->fetch_assoc()) {
      ...
      $result2 = mysqli_query($db,$sql2);  
    	...
    	if(($dateDBSq >= $dateSq) && ($dateDBSq <= $dateEq)) {
         //echo "<tr><td>" . $row["ServiceID"]. "<br>(" . $row["Cadre"]. ")" . "</td><td>" . $row["Name"]. "</td><td>" . $row["Designation"] . "<br>(Grade-" . $row["Grade"] . ") " . "</td><td>" . $row["Workplace"] . ", " . $row["Office"] . "</td><td>" . $row["DestinationCountry"] . "</td><td>" . $row["FundingSource"] . "</td><td>" . $row["Purpose"] . "</td><td>" . $row["StartDate"]. "<br>( ". $row["ActualDeparture"]. ")" . "</td><td>" . $row["EndDate"] . "<br>(" . $row["ActualArrival"]. ")" . "</td><td>" . $row["Days"] . "</td><td>" . '<a href="./uploads/'.$row["GO"].'" target="_blank"> Click </a>'; 
