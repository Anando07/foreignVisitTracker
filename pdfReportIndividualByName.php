...
$pdf->Ln();	
//$pdf->Cell(0,10,'Foreign Visit Record by Name',0.0,0.0,'C');
$pdf->Cell(0,10,'Foreign Visit Record of '.$nameReq. " from ". $startDate. " to ".$endDate,1.0,0.0,'C');
$pdf->SetFont('Arial','',8);
$pdf->Ln();	
...
if($query->num_rows > 0)
{
	...
	while($row = $query->fetch_assoc()) { 
		...
		//$pdf->Cell(45,7,$row["Designation"]. " (Grade-" . $row["Grade"] . ")",1,0,'C');
		$pdf->Cell(60,7,$row["Designation"]. " (Grade-" . $row["Grade"] . "), " . $row["Office"], 1,0,'C');
		//$pdf->Cell(51,7,$row["Workplace"] . ", " . $row["Office"],1,0,'C');
