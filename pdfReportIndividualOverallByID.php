...
$idOverallReqEncoded = $_GET['idOverallReq'];   
$idOverallReq = urldecode($idOverallReqEncoded);

$sql = "SELECT * from ForeignVisit WHERE ServiceID = " . $idOverallReq . " ORDER BY StartDate desc";
...
//$pdf->SetTextColor(255,0,0); 
//$pdf->Cell(0,10,'Foreign Visit Record by Name',0.0,0.0,'C');
$pdf->Cell(0,10,'Foreign Visit Record of Service ID: '.$idOverallReq,1.0,0.0,'C');
$pdf->SetFont('Arial','',8);
$pdf->Ln();	
...
if($query->num_rows > 0)
{
	...
	while($row = $query->fetch_assoc()) { 
		...
		//$pdf->Cell(20,7,$row["EndDate"],1,0,'C');
